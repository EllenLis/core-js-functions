...
function getPowerFunction(exponent) {
  const numbInPower = function powerFunction(number) {
...
function getPolynom(...coefficients) {
  ...
  const polynomial = function polynomialFunction(x) {
...
function logger(func, logFunc) {
  return function wrappedFunction(...args) {
    ...
    try {
      ...
      logFunc(`Error in ${funcName}: ${error}`);
      throw error;
    }
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function acceptSubsequentArgs(...args2) {
...
function getIdGeneratorFunction(startFrom) {
  ...
  function incrementAndReturnId() {
    currentId += 1;
    return currentId;
  }

  return incrementAndReturnId;
}
